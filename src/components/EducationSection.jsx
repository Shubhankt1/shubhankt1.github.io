...
const EducationSection = ({ index, sectionWidth, sectionLeft, isVisible }) => (
  <section
    className="min-h-screen bg-white sticky top-0 px-8 py-16 flex flex-col justify-center shadow-xl"
    style={{
      width: `${sectionWidth}%`,
      left: `${sectionLeft}%`,
  ...
  >
    <div
      ...
      `}
    >
      <h2 className="section-heading text-black mb-12">Education</h2>
      <div className="grid grid-cols-1 tablet-lg:grid-cols-2 gap-8">
        {educationData.map((ed) => (
          <div
            key={ed.degree}
            className="bg-[#1a1a1a] rounded-2xl p-6 shadow-xl transition-transform hover:scale-105"
          >
            <h3 className="text-xl font-semibold text-white mb-1">
              {ed.degree}
            </h3>
            <p className="text-sm text-gray-400 mb-1">{ed.school}</p>
